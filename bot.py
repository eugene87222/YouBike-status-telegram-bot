...
async def welcome(message: types.Message):
    msg = '*用途*：查詢 YouBike 站點狀態\n\n'
    msg += '*使用方法一*：輸入一個或多個關鍵字查詢，關鍵字間用空白分隔\n'
    msg += '例如：`清華大學`、`交通大學 大學路`\n'
    msg += '查詢的欄位有：\n- 站點名稱\n- 站點所在區域\n- 站點地址\n\n'
    msg += f'*使用方法二*：使用 Telegram 傳送 GPS 座標，將回傳方圓 {get_radius(None)} 公尺內最近的五個站的站點狀態\n\n'
    msg += '*其他*\n'
...
async def radius(message: types.Message):
    ...
    if len(args) == 0:
        ...
        save_radius(message.from_user.id, int(args))
        await message.answer(f'搜尋半徑已更新為 {int(args)} 公尺')
...
async def search(message: types.Message):
    if message.content_type == ContentType.ANIMATION:
        ...
        status = await message.answer(f'正在取得方圓 {get_radius(message.from_user.id)} 公尺內最近的五個 YouBike 站點資訊')
        ...
        status = await message.answer('正在取得 YouBike 站點資訊')
        youbike1_stations = get_all_stations(youbike_type=1)
        youbike2_stations = get_all_stations(youbike_type=2)
